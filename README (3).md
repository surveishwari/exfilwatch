...
**[Live demo](https://exfilwatch.onrender.com)** · **[GitHub](https://github.com/YOUR_USERNAME/exfilwatch)** · [Quick start](#quick-start-verified) · [Demo flow](#2-minute-demo-flow) · [Architecture](#architecture) · [Self red-team](#we-attacked-our-own-detector-live) · [API](#api--one-line-integration)

</div>

---

## Problem → Threat → Gap, in three lines

- **Problem:** AI assistants now have tool access — email, databases, internal APIs — and their replies are trusted as plain text.
- **Threat:** A reply can carry a secret invisibly — hidden Unicode, or a zero-click markdown image URL — while the visible words stay completely normal. **GlassWorm**, a supply-chain worm active since October 2025 using exactly this technique, had spread to 400+ compromised packages across npm, OpenVSX, and the VS Code Marketplace by March 2026.
- **Gap:** Nothing standard reads the *output* channel. Prompt-injection filters check inputs. DLP scanners check visible plaintext. Content moderation checks toxicity. None of them decode invisible characters or resolve a suspicious URL — a covert channel sails through every one of them clean.
...
|---|---|---|
| Precision | **96.8%** | Of everything flagged, 96.8% was truly malicious |
| Recall | **100%** | All 30 malicious samples were caught |
| False positive rate | **4.3%** | 1 of 23 benign samples wrongly flagged |
| Accuracy | **98.1%** | 52 of 53 samples classified correctly |

**Corpus:** 53 labeled samples = 30 malicious (both covert schemes + sensitive-data leaks) + 23 benign (ordinary AI-reply-style sentences).
...
```bash
git clone https://github.com/YOUR_USERNAME/exfilwatch.git && cd exfilwatch
...
Every regulated industry deploying AI agents — finance, healthcare, SaaS with internal copilots — needs an answer to "how do you know your AI didn't leak anything?" Today that answer is usually "we trust the model." ExfilWatch turns it into "here's the gate, here's the audit chain, here's the number, verify it yourself." That shift — from trust to proof — is what makes this a product category, not a hackathon toy.
